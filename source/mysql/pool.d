            ...
            uint maxConnections, uint initialConnections, uint incrementalConnections, uint waitSeconds) shared
    {
        _pool = cast(shared Tid)spawn(
                new shared Pool(host, port, user, password, database, charset,
                        maxConnections, initialConnections, incrementalConnections, waitSeconds.dur!"seconds"));
...
/++
// Modify the database connection information to a real MySQL server for unittest.
unittest
{
   import core.thread;
   import std.stdio;

   ConnectionPool pool = ConnectionPool.getInstance("127.0.0.1", 3306, "root", "root", "mysql_d_testing", null, 5, 3);
